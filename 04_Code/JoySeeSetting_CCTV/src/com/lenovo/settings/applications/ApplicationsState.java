...
public class ApplicationsState {
    ...
    /**Add for sanao project*/
//    private String ForSanAo = "com.sanao";
//    private boolean isSanAo = false;
    ...
    public static class AppEntry extends SizeInfo {
        ...
        public long getLastDate(Context context, PackageManager pm) {
            long date = getAppLastDateFromProvider(context, info.packageName);
            if (date == 0) {
                Log.e(TAG, "provider's last date is null");
                try {
                    date = pm.getPackageInfo(info.packageName, 0).lastUpdateTime;
        ...
        boolean ensureIconLocked(Context context, PackageManager pm) {
            if (this.apkFile.exists()) { // Rony add for update last date time     
            	this.time = getLastDate(context,pm);
            }
            if (this.icon == null) {
                if (this.apkFile.exists()) {
                    this.icon = this.info.loadIcon(pm);
                    /*try {
						this.time = pm.getPackageInfo(info.packageName, 0).firstInstallTime;
					} catch (NameNotFoundException e) {
         ...
         public void onReceive(Context context, Intent intent) {
             ...
             if (Intent.ACTION_PACKAGE_ADDED.equals(actionStr)) {
                 ...
                 if (avail) {
                     ...
                     for (String pkgName : pkgList) {
                    	 requestSize(pkgName);
	                	 //Log.e(TAG,"pkgName = "+pkgName+" AppEntry size = "+entry.size);
                     }
                 }
             }
         }
    ...
    void resume(Callbacks callbacks) {
        ...
        synchronized (mEntriesMap) {
            ...
            Log.d(TAG, "-------------- mApplications size = " + mApplications.size());
            ...
            //Add for sanao in 2012-08-01
//            if(isSanAo){
//                List<ApplicationInfo> sanaoList = new ArrayList<ApplicationInfo>();
//                for (ApplicationInfo appInfo : mApplications) {
//                    if (appInfo.packageName.trim().startsWith(ForSanAo)) {
//                        Log.d(TAG," remove packageName = " + appInfo.packageName);
//                        sanaoList.add(appInfo);
//                    }
//                }
//                mApplications.removeAll(sanaoList);
//            }
    ...
    void ensureIcon(AppEntry entry) {
        /*if (entry.icon != null) {
            return;
        }*/ //Rony remove for icon and last time sync
        synchronized (entry) {
    ...
    private long getTotalExternalSize(PackageStats ps) {
        if (ps != null) {
            return ps.externalCodeSize + ps.externalDataSize
                    + ps.externalMediaSize + ps.externalObbSize + ps.externalCacheSize;
        }
        return SIZE_INVALID;
    }
                ...
                synchronized (mEntriesMap) {
                    ...
                    if (entry != null) {
                        synchronized (entry) {
                            ...
                            long newSize = getTotalExternalSize(stats)
                            ...
                            if (entry.size != newSize ||
                                ...
                                entry.size = newSize;
                                entry.cacheSize = stats.cacheSize + stats.externalCacheSize;
                                entry.codeSize = stats.codeSize + stats.externalCodeSize;
                                entry.dataSize = stats.dataSize + stats.externalDataSize;
                                entry.externalCodeSize = externalCodeSize;
                                entry.externalDataSize = externalDataSize;
                                ...
                                if (DEBUG) Log.i(TAG, "Set size of " + entry.label + " " + entry
                                        + ": " + entry.sizeStr);
                                MyLog(" onGetStatsCompleted = " + entry.label
                                        + "mTotalSize = " + entry.size
                                        + " mAppEntry.codeSize = "
                                        + entry.codeSize + " mDataSize = "
                                        + entry.dataSize + " mCacheSize = "
                                        + entry.cacheSize + " externalSize = "
                                        + entry.externalSize
                                        + " internalSize = "
                                        + entry.internalSize
    ...
    public static long getAppLastDateFromProvider(Context context, String packageName) {
        long date = 0;
        Cursor cursor = context.getContentResolver().query(
                InstalledAppDetails.CONTENT_URI,
                null,
                InstalledAppDetails.COLUMN_NAME_PACKAGE_NAME + " = ?",
                new String[] {
                    packageName
                },
                null);
            ...
            if (cursor.getCount() != 0) {
                ...
                date = cursor.getLong(
                        cursor.getColumnIndex(
                                InstalledAppDetails.COLUMN_NAME_LAST_USED_DATE));
            }
            cursor.close();
        }
        return date;
