...
//                 tDBUtils.que
        	...
        	DBUtils tDBUtils = new DBUtils(context);
            ApplicationBean tBean = tDBUtils.queryApplicationByPkgName(packageName);
            AppLog.d(TAG, "@@@packageName="+packageName+"   this.pk="+context.getPackageName());
            if(packageName.equals(context.getPackageName())){
                tDBUtils.deleteOneTaskByPkgName(packageName);
            }
            if(null!=tBean){
	...
	Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
        	...
        	case 3:
                ...
                TaskBean updateBean=dbu.queryTaskByPkgName(mContext.getPackageName());
                if(updateBean!=null&&updateBean.getDownloadSize()>=updateBean.getSumSize()&&updateThread==null&&!update){
                    Log.d(TAG, "--------UpdateThread---install--------");
                    update=true;
