...
public class MainActivity extends BaseActivity implements View.OnClickListener,View.OnFocusChangeListener{
	...
	private ArrayList<ImageView> starArray=new ArrayList<ImageView>();
	...
	private static final int MSG_UPDATE_DIALOG=3;
	private static final int MSG_CLOSE_UPDATE_DIALOG=4;
	private static final int MSG_CLOSE_ACTIVITY=5;
	private static final int MSG_SHOWUPDATEDIALOG_TRUE = 6;
    ...
    private  ImageView pageView4;
    private  ImageView pageView5;
    ...
    private TaskBean joyseeBean;
    private UpdateThread mThreadUpdate;
...
//	 }
	...
	private void setupViews() {
		...
		titleImage=(TextView)this.findViewById(R.id.top2_title);
...
//		mDownloadRecord.setNextFocusDownId(mMoreLayout.getId());
		...
		titleImage.setText(this.getResources().getString(R.string.top_title_recommend));
	...
	public Handler mHandler=new Handler(){
		public void handleMessage(Message msg) {
            ...
            case MSG_UPDATE_DIALOG:
            	AppLog.d(TAG, "mhandler MSG_UPDATE_DIALOG");
            	stopProgressDialog();
            	showUpdateDialog(false);
            	break;
            case MSG_CLOSE_UPDATE_DIALOG:
            	if(updateDialog!=null){
            		updateDialog.dismiss();
            	}
            	break;
            case MSG_CLOSE_ACTIVITY:
                if(Constants.enforce)
                    finish();
            	break;
            case MSG_SHOWUPDATEDIALOG_TRUE:
            	stopProgressDialog();
...
//			control.firstTime=System.currentTimeMillis();
    		if(keyCode==KeyEvent.KEYCODE_DPAD_DOWN||keyCode==KeyEvent.KEYCODE_DPAD_LEFT||
    			...
    			/** 下载记录 按键向右 焦点不动 */
    			if(keyCode == event.KEYCODE_DPAD_RIGHT && mDownloadRecord.hasFocus()){
    				return true;
    			}
	...
	public void nextFlipperPage(boolean fastMode){
		...
		if(!mMoreLayout.hasFocus()){
			...
			if(GridLayout.class.isInstance(currentView)){
				...
				titleImage.setText(this.getResources().getString(R.string.top_title_recommend));
				pageSearch.setBackgroundResource(R.drawable.search_small);
			}else{
				if(edit!=null){
				...
				titleImage.setText(this.getResources().getString(R.string.top_title_search));
...
//			mViewFlipper.setOutAnimation(AnimationUtils.loadAnimation(MainActivity.this, R.anim.left_out));
    ...
    public void initPage(){
     	starArray.clear();
     	pageView1= (ImageView) findViewById(R.id.home_page1);
     	pageView2= (ImageView) findViewById(R.id.home_page2);
     	pageView3= (ImageView) findViewById(R.id.home_page3);
     	pageView4= (ImageView) findViewById(R.id.home_page4);
     	pageView5= (ImageView) findViewById(R.id.home_page5);
...
//		AppLog.d(TAG, "pageNum="+pageNum+"---pageTotal="+pageTotal);
	   for(int i=0;i<pageTotal;i++){
		   if(pageNum==i){
			   ImageView currentPageView= starArray.get(i);
			   String mNum=""+(i+1)+"";
...
//			   starArray.get(i).setText("");
	...
	public void addSearchViewFillper(){
    	...
    	search = (Button) view.findViewById(R.id.search_button_search);
	...
	protected Dialog onCreateDialog(int id) {
		 switch (id) {
			case URLINPUT_DIALOG:
				LinearLayout linearLayout = (LinearLayout) getLayoutInflater().inflate(R.layout.url_dialog, null);
...
//								        (tGrid.getChildAt(j).findViewById(R.id.pop_update)).setVisibility(View.GONE);
				...
				if(arg1.getStringExtra("pkg_name").equals(MainActivity.this.getPackageName())){
	            	...
	            	if(file_sum==0){
	                    ...
	                    updateProgressBar.setProgress(res);
	                	if(res>=100){
	                		mHandler.sendEmptyMessageDelayed(MSG_CLOSE_ACTIVITY, 5000);
	                	}
	                }
					return;
				}
...
//    			(view.findViewById(R.id.pop_update)).setVisibility(View.GONE);
    ...
    Dialog updateDialog;
    private AppsBean oldApp = new AppsBean();//升级用的
	private AppsBean upApp = new AppsBean();//升级用的
	public class UpdateThread extends Thread {
		private final static String TAG = "com.joysee.appstore.MainActivity.UpdateThread";
		private boolean flag = true;
		private String verName;
		private int verCode;
		DataOperate operate;
		AppsBean mRes;// 服务器返回的结果

		public UpdateThread() {
			operate = new DataOperate(MainActivity.this);
		}
		...
		public void run() {
			...
			try {
				packInfo = pm.getPackageInfo(MainActivity.this.getPackageName(),PackageManager.GET_ACTIVITIES);
				verCode = packInfo.versionCode;
				verName = packInfo.versionName;
				AppLog.d(TAG, "verCode=" + verCode + ";verName=" + verName);
				oldApp.setVersion("" + verCode);
				mRes = operate.getAppStoreUpdate(MainActivity.this.getPackageName(), "" + verCode);
				Constants.enforce = false;
				if (null != mRes) {
					if (mRes.getPkgName().equals(MainActivity.this.getPackageName())) {
						upApp.setAppName(mRes.getAppName());
						upApp.setPkgName(mRes.getPkgName());
						upApp.setApkUrl(mRes.getApkUrl());
						upApp.setVersion(mRes.getVersion());
						upApp.setSerImageUrl(null);
						int force = mRes.getUpdateTag();
						upApp.setUpdateTag(force);
						int updateVerCode = Integer.valueOf(mRes.getVersion());
						AppLog.d(TAG, "----updateVerCode="+ updateVerCode + ";verCode=" + verCode +"   force = "+force);
						AppLog.d(TAG, "down url = "+upApp.getApkUrl());
						if (force == 1) {
							Constants.enforce = true;
							mHandler.sendEmptyMessage(MSG_SHOWUPDATEDIALOG_TRUE);
						}else if(force == 0){
							Constants.enforce = false;
				...
				e.printStackTrace();
				AppLog.d(TAG, "--- UpdateThread NameNotFoundException ---");
			}
		}
		public boolean isFlag() {
			return flag;
    ...
    public TextView progressTex,info;
	ServiceConnection conn;
	public void showUpdateDialog(boolean isDown){
		AppLog.d(TAG, "UPDATE_DIALOG");
		updateDialog=new Dialog(this,R.style.update_dialog);
		
		updateDialog.getWindow().setLayout(1280, 720);
		
		RelativeLayout updateLayout = (RelativeLayout) getLayoutInflater().inflate(R.layout.update_dialog,null);
		info=(TextView)updateLayout.findViewById(R.id.updateInfo);
		updateProgressBar=(ProgressBar)updateLayout.findViewById(R.id.progress_bar);
		progressTex=(TextView)updateLayout.findViewById(R.id.progress_text);
		if(isDown){
			info.setVisibility(View.GONE);
			long downSize=joyseeBean.getDownloadSize();
			long sumSize=joyseeBean.getSumSize();
			int res = 0;
			if(sumSize!=0){
				res=(int)(downSize*100/sumSize);
			}
        	updateProgressBar.setProgress(res);
			updateProgressBar.setVisibility(View.VISIBLE);
			progressTex.setVisibility(View.VISIBLE);
		}else{
			updateProgressBar.setVisibility(View.GONE);
			progressTex.setVisibility(View.GONE);
			if(Constants.enforce){
				info.setVisibility(View.GONE);
				updateProgressBar.setVisibility(View.VISIBLE);
				progressTex.setVisibility(View.VISIBLE);
		...
		if(!isDown){
			if(CaCheManager.checkSDcard()){
		        ...
		        upApp.setDownloadDir(downDir.getAbsolutePath());
			}else{
				File downDir = new File(Constants.APKROOT);
				upApp.setDownloadDir(downDir.getAbsolutePath());
			}
		}else{
			if(null!=joyseeBean && null!=joyseeBean.getPkgName()){
				upApp.setAppName(joyseeBean.getAppName());
				upApp.setPkgName(joyseeBean.getPkgName());
				upApp.setApkUrl(joyseeBean.getUrl());
				upApp.setVersion(joyseeBean.getVersion());
				upApp.setSerImageUrl(joyseeBean.getIconUrl());
				upApp.setTypeName(joyseeBean.getTypeName());
			}
			if(CaCheManager.checkSDcard()){
				File baseDir = Environment.getExternalStorageDirectory();
		        File downDir = new File(baseDir, "joysee");
		...
		conn=new ServiceConnection() {
			...
			public void onServiceConnected(ComponentName name,IBinder service) {
				...
				try{
	        		...
	        		if(downloadService!=null && null!=upApp){
						downloadService.startDownload(upApp);
						AppLog.d(TAG, "------------------startDownload------------------");
					}else{
						AppLog.d(TAG, "----------------downloadService or appsBean is null--------------");
					}
	        	}catch(Exception e){
	        	    AppLog.d(TAG, "-------------------error e="+e.getMessage());
		...
		Intent intent = new Intent("com.joysee.appstore.service.DownloadService");
		bindService(intent, conn, BIND_AUTO_CREATE);
		Log.d(TAG, "downloadDir="+upApp.getDownloadDir());
		...
		updateDialog.setContentView(updateLayout);
		updateDialog.show();
		if(!Constants.enforce){
			mHandler.sendEmptyMessageDelayed(MSG_CLOSE_UPDATE_DIALOG, 5000);
		}
	}
	
	private void checkUpdate(){
		
		AppLog.d(TAG, ":::::::checkUpdate:::::::::");
		
		joyseeBean = mDBUtils.queryTaskByPkgName(this.getPackageName());
        if(joyseeBean==null||joyseeBean.getPkgName()==null){
        	AppLog.d(TAG, "UpdateThread.start()");
        	mThreadUpdate=new UpdateThread();
            mThreadUpdate.start();
        }else{
        	try {
        		pm=MainActivity.this.getPackageManager();
        		packInfo=pm.getPackageInfo(MainActivity.this.getPackageName(), PackageManager.GET_ACTIVITIES);
            	int downVerCode=Integer.valueOf(joyseeBean.getVersion());
            	AppLog.d(TAG, "--------downVerCode="+downVerCode+";packInfo.versionCode="+packInfo.versionCode+";downsize="+joyseeBean.getDownloadSize()+";sumsize="+joyseeBean.getSumSize());
    			if(Constants.enforce){
    				mHandler.sendEmptyMessage(MSG_SHOWUPDATEDIALOG_TRUE);
    			}else{
    			    upApp.setAppName(joyseeBean.getAppName());
    		        ...
    		        Intent intent2 = new Intent("com.joysee.appstore.service.DownloadService");
    		        bindService(intent2, conn, BIND_AUTO_CREATE);
    			}
			} catch (NameNotFoundException e) {
				e.printStackTrace();
				AppLog.d(TAG, "---NameNotFoundException---");
			}
        }
	}
}