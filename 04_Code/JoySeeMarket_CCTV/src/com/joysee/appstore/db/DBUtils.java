...
//            	}
    ...
    public int deleteOneTask(TaskBean tBean){
    	...
    	AppLog.d(TAG, "deleteOneTask--path="+tBean.getTmpFileName());
    	CaCheManager.clearDataCache(tBean.getTmpFileName());
    	return ret;
    }
...
//        	List<TaskBean> data  =queryCompletedTask();//TODO here may be need optimize by yuhongkun
        	...
        	if(taskBean==null||taskBean.getPkgName()==null){
        	    Log.d(TAG, "-----------taskbean is null id="+tBean.getId());
        	    return false;
        	}
//				Log.d(TAG, "***********updateprogress()=====task="+taskBean.getPkgName()
//						+"=======downloadSize="+taskBean.getDownloadSize()+"******sumsize="+taskBean.getSumSize());
			if(taskBean.getStatus()==Constants.DOWNLOAD_STATUS_PAUSE){
			    Log.d(TAG, "-------task is pause------");
			    return true;
			}
...
//            	int index =data.indexOf(tBean);
        		...
        		if(taskBean.getPkgName().equals(mContext.getPackageName())&&!Constants.update&&!Constants.enforce){//应用商城自己升级只有退出首页后才另外发广播进行安装
        			Log.d(TAG, "---------"+mContext.getPackageName()+"--down completed");
        			return true;
        		}
        		if(needStartService("com.joysee.appstore.service.DownloadService")){
        			...
        			Log.d(TAG, "updateProgress---begin---srartService");
...
//        	}
	...
	public boolean needStartService(String serviceName) {
		...
		Log.d(TAG, "needStartService--size="+runningService.size());
		for (int i = 0; i < runningService.size(); i++) {
			if (runningService.get(i).service.getClassName().toString().equals(serviceName)) {
				Log.d(TAG, "needStartService---is start");
				return true;
			}
		}
		Log.d(TAG, "needStartService---is deal");
		return false;
	}
...
//    }
    ...
    /**
     * query error task with not manual pause 
     * @return list TaskBean
     */
    public List<TaskBean> queryDownErrorTask(){
    	List<TaskBean> data = new ArrayList<TaskBean>();
    	List<ThreadBean> threadList = new ArrayList<ThreadBean>();
    	Cursor taskCursor=null;
    	Cursor threadCursor = null;
    	 try{
    	ContentResolver contentResolver = mContext.getContentResolver();
    	String whereClause = DownloadTaskColumn.STATUS +"=="+Constants.DOWNLOAD_STATUS_ERROR +" and "+DownloadTaskColumn.DOWNLOADSIZE+"<"+DownloadTaskColumn.SUMSIZE;
    	taskCursor =contentResolver.query(DownloadTaskColumn.CONTENT_URI, null,whereClause , null,null);
    	while(taskCursor.moveToNext()){
    		TaskBean tTaskBean = new TaskBean();
            int id = taskCursor.getInt(taskCursor.getColumnIndex(DownloadTaskColumn.ID));
            ...
            int sumSize = taskCursor.getInt(taskCursor.getColumnIndex(DownloadTaskColumn.SUMSIZE));
            int downloadLength =taskCursor.getInt(DownloadTaskColumnIndex.DOWNLOADSIZE);
            AppLog.d(TAG, "&&&&&&&&&&&&&queryDownErrorTask----------------------appName="+name+";downloadLength="+downloadLength+";sumSize="+sumSize);
            int status = taskCursor.getInt(taskCursor.getColumnIndex(DownloadTaskColumn.STATUS));
            String url = taskCursor.getString(taskCursor.getColumnIndex(DownloadTaskColumn.URL));
            String iconUrl = taskCursor.getString(taskCursor.getColumnIndex(DownloadTaskColumn.ICONURL));
            byte[] icon = taskCursor.getBlob(taskCursor.getColumnIndex(DownloadTaskColumn.ICON));
    ...
    public List<TaskBean> queryUnCompletedTask(){
    	...
    	Cursor threadCursor = null;
    	 try{
    	ContentResolver contentResolver = mContext.getContentResolver();
    	String whereClause = DownloadTaskColumn.PKGNAME+"!='"+mContext.getPackageName()+"' and "+DownloadTaskColumn.STATUS +"!="+Constants.DOWNLOAD_STATUS_PAUSE +" and "+DownloadTaskColumn.DOWNLOADSIZE+"<"+DownloadTaskColumn.SUMSIZE;
    	taskCursor =contentResolver.query(DownloadTaskColumn.CONTENT_URI, null,whereClause , null,null);
    	while(taskCursor.moveToNext()){
    		TaskBean tTaskBean = new TaskBean();
            int id = taskCursor.getInt(taskCursor.getColumnIndex(DownloadTaskColumn.ID));
