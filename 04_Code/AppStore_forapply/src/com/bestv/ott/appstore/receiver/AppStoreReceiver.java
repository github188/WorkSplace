...
public class AppStoreReceiver extends BroadcastReceiver {
    ...
    public IApkManageService apkService =null;
        		...
        		mng.connect(new IBesTVOttConnListener() {
        			public void onBesTVOttServiceConnected() {//中间件 is connection
        				...
        				if (null != userProfile && null != userProfile.getUserID()) {
        					Editor editor = mContext.getSharedPreferences("user_information", Context.MODE_PRIVATE).edit();
        					editor.putString("userID", userProfile.getUserID()).commit();
        					editor.putString("groupID", userProfile.getUserGroup()).commit();
        				}
        				AppLog.d(TAG, "----------- ------apkService---------connect");
        				apkService = BesTVOttServices.createApkManageService();
        				AppLog.d(TAG, "----------- ------apkService--------------connect--------apkService=-"+apkService);
        				while(apkService==null){
        					try {
        						Thread.sleep(2000);
        					} catch (InterruptedException e) {
        						e.printStackTrace();
        					}
        					AppLog.d(TAG, "-----------------apkService----------is--------connect---------");
        					apkService = BesTVOttServices.createApkManageService();
        				}
        			}
        		});
