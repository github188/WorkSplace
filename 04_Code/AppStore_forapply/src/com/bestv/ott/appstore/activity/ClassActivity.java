...
public static final String TAG = "com.joysee.appstore.ClassActivity";
    ...
    public void setupViews(){
        ...
        mDownRecord=(DownloadTextView)this.findViewById(R.id.top_down_record);
        ...
        mMenuLinearLayout = (CategoryLayout) this.findViewById(R.id.class_menu_linearlayout);
    ...
    public Handler mainHandler=new Handler(){
        public void handleMessage(Message msg) {
            ...
            case MSG_HIDE_UPDATE:
            	...
            	break;
            case MSG_FOCUS:
            	if(AnimationButton.class.isInstance(msg.obj)){
            		((AnimationButton)msg.obj).requestFocus();
            		control.transformAnimation(mImageView, mImageView, ((AnimationButton)msg.obj), ClassActivity.this, false, true);
            		((AnimationButton)msg.obj).startAnimation();
            	}
            	break;
            }
...
//    	}
		...
		mGridLayout = new CenterAppLayout(this);
		mGridLayout.setBackImageView(mImageView);
		mGridLayout.setFocusGroup(mRootLayout);
		
		mGridLayout.setColumnCount(4);
		for (int i = 0; i < listSize; i++) {
			View view = mLayoutInflater.inflate(R.layout.class_item_layout,null);
			AnimationButton button = (AnimationButton) view.findViewById(R.id.class_item_img);
			button.setOnClickListener(this);
...
//		}
		...
		for(int i=0;i<appList.size();i++){
			...
			AnimationButton ab=(AnimationButton)view.findViewById(R.id.class_item_img);
			...
			AppLog.d(TAG, "---------------res="+res+";(float)res="+(float)res+";pkgname="+appList.get(i).getPkgName()+";appStatus="+appStatus+";name="+appList.get(i).getAppName());
			switch(appStatus){
...
//				ab.requestFocus();
                ...
                mainHandler.sendMessageDelayed(fousMes, 80);
			}
		}
...
//				TextButton tb=null;
//				if(i==0){
//					tb=(MenuButton)viewM.findViewById(Constants.MENU_LEFT_ID);
//				}else if(i==mMenuLinearLayout.getChildCount()-1){
//					tb=(MenuButton)viewM.findViewById(Constants.MENU_RIGHT_ID);
//				}else{
//					tb=(MenuButton)viewM.findViewById(R.id.menu_item_img);
//				}
//				if(tb!=null&&((Integer)tb.getTag())==typeID){
//					AppLog.d(TAG, "--------------------------");
//					tb.requestFocus();
...
//		}
		...
		mGridLayout.setOnSlidingAtEndListener(new CenterAppLayout.OnSlidingAtEndListener() {
...
//		if(appList.size()==1){
//			AnimationButton button=(AnimationButton) mGridLayout.getChildAt(0).findViewById(R.id.class_item_img);
//			button.setOnKeyListener(new LeftAndRightListener());
//		}else if(appList.size()>1&&appList.size()<5){
//			AnimationButton button1=(AnimationButton) mGridLayout.getChildAt(0).findViewById(R.id.class_item_img);
//			button1.setOnKeyListener(new LeftListener());
//			AnimationButton button2=(AnimationButton) mGridLayout.getChildAt(appList.size()-1).findViewById(R.id.class_item_img);
//			button2.setOnKeyListener(new RightListener());
//		}else if(appList.size()>=5&&appList.size()<=8){
//			AnimationButton button1=(AnimationButton) mGridLayout.getChildAt(0).findViewById(R.id.class_item_img);
//			button1.setOnKeyListener(new LeftListener());
//			AnimationButton button2=(AnimationButton) mGridLayout.getChildAt(3).findViewById(R.id.class_item_img);
//			button2.setOnKeyListener(new RightListener());
//			AnimationButton button3=(AnimationButton) mGridLayout.getChildAt(4).findViewById(R.id.class_item_img);
//			button3.setOnKeyListener(new LeftListener());
//			AnimationButton button4=(AnimationButton) mGridLayout.getChildAt(appList.size()-1).findViewById(R.id.class_item_img);
...
//		}
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
    	AppLog.d(TAG, "mPressTime"+mPressTime);
...
//	        	}
    ...
    protected void onDestroy(){
    	if(null!=mGridLayout){ //清空最后一屏bitmap所占内存
    		for(int  i=0;i<mGridLayout.getChildCount();i++ ){
    			AnimationButton button=(AnimationButton)mGridLayout.getChildAt(i).findViewById(R.id.class_item_img);
    			BitmapDrawable drawable = (BitmapDrawable) button.getImageView().getDrawable();
				Bitmap bmp = drawable.getBitmap();
				if (null != bmp && !bmp.isRecycled()){
					bmp.recycle();
				    bmp = null;
				    AppLog.d(TAG, "-----------Bitmap.recycle()-----------");
				}
    		}
    	}
