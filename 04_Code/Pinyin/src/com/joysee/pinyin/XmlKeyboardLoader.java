...
public class XmlKeyboardLoader {
        ...
        try {
            ...
            while (mXmlEventType != XmlResourceParser.END_DOCUMENT) {
                ...
                if (mXmlEventType == XmlResourceParser.START_TAG) {
                    ...
                    if (XMLTAG_SKB_TEMPLATE.compareTo(attribute) == 0) {//skb_template
                    ...
                    } else if (XMLTAG_KEYTYPE.compareTo(attribute) == 0) {//key_type
                        ...
                        lastKeyTypeId = id;
                    } else if (XMLTAG_KEYICON.compareTo(attribute) == 0) {//key_icon
                    ...
                    } else if (XMLTAG_KEY.compareTo(attribute) == 0) {//key
            ...
            while (mXmlEventType != XmlResourceParser.END_DOCUMENT) {
                ...
                if (mXmlEventType == XmlResourceParser.START_TAG) {
                    ...
                    if (XMLTAG_KEYBOARD.compareTo(attr) == 0) {//<keyboard>
                        ...
                        mSkbTemplate = skbPool.getSkbTemplate(skbTemplateId,//搞一个skb的模板,搞出来一个以后存到池子里。
                                mContext);
                    ...
                    } else if (XMLTAG_ROW.compareTo(attr) == 0) {//<row>
                        ...
                        for (int i = 0; i < labelArr.length; i++) {//将所有的keys加入到softkeyboard中
                            softKey = new SoftKey();
                            int keyCode = 0;
                            if (null != codeArr) {
    ...
    private SoftKey getSoftKey(XmlResourceParser xrp,
            ...
            IOException {
        int keyCode = getInteger(xrp, XMLATTR_KEY_CODE, 0);// 0
        String keyLabel = getString(xrp, XMLATTR_KEY_LABEL, null);// ,
        Drawable keyIcon = getDrawable(xrp, XMLATTR_KEY_ICON, null);// @drawable/comma_full_icon
        Drawable keyIconPopup = getDrawable(xrp, XMLATTR_KEY_ICON_POPUP, null);// @drawable/comma_full_popup_icon
