...
import com.joysee.pinyin.PinyinIME.DecodingInfo;
...
public class CandidatesContainer extends RelativeLayout implements
    AnimationListener, ArrowUpdater {
    /**
...
//        mRightArrowBtn.setOnTouchListener(this);
    ...
    public void showCandidates(PinyinIME.DecodingInfo decInfo,
        ...
        PinyinLog.d(TAG, "enter showCandidates()!");
        ...
        PinyinLog.d(TAG, "showCandidates(), mCurrentPage = "+mCurrentPage+" enableActiveHighlight = "+enableActiveHighlight);
...
//        measuredWidth = 600;
    ...
    public boolean activeCurseForward() {
        ...
        if (cv.activeCursorForward()) {
            cv.invalidate();
            PinyinLog.d(TAG , "call cv.activeCursorForward()");
            return true;
        } else {
            PinyinLog.d(TAG , "call pageForward()");
    ...
    public boolean pageForward(boolean animLeftRight,
            boolean enableActiveHighlight) {
        PinyinLog.d(TAG, "pageForward()");
        if (null == mDecInfo) return false;

        if (mFlipper.isFlipping() || !mDecInfo.preparePage(mCurrentPage + 1)) {
            PinyinLog.d(TAG, "mFlipper.isFlipping() || !mDecInfo.preparePage(mCurrentPage + 1)  return false");
            return false;
        }
