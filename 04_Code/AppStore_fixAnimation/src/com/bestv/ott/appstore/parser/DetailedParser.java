...
public class DetailedParser extends BaseParser<AppsBean>{
	...
	public static final String TAG_CONTENTPROVIDER = "contentProvider";
	...
	public AppsBean parseJSON(String paramString, PageBean pageBean)
        ...
        if(appObj.has(TAG_CONTENTPROVIDER)){
        	appBean.setContentProvider(appObj.getString(TAG_CONTENTPROVIDER));
        }else{
        	appBean.setContentProvider("");
        }
        appBean.setApkUrl(RequestParam.SERVICE_IMAGE_URL+appObj.getString(TAG_FILEPATH));
        ...
        appBean.setPromotionPrice(appObj.getDouble("promotionPrice"));//促销价
        appBean.setAppProductCode(appObj.getString("appProductCode"));
        appBean.setAppPackageProductCode(appObj.getString("appPackageProductCode"));
        appBean.setBoutique(appObj.getBoolean("boutique"));
        appBean.setPeripherals(appObj.getString("peripherals"));
        appBean.setOriginalPrice(appObj.getDouble("originalPrice"));//原价
        appBean.setAppPackageId(appObj.getString("appPackageId"));
        if(appObj.has("serviceCode")){
        	appBean.setServiceCode(appObj.getString("serviceCode"));
        }
		return appBean;
	}
