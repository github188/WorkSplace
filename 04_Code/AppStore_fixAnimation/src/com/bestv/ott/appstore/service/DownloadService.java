...
//	private Map<String, Integer> taskStatusMap = new ConcurrentHashMap<String, Integer>();
    ...
    private PayMng mng = null; 
    
    private ApkInfoService apkService =null;
...
//		workHandler.sendMessage(message);
		...
		new Thread(){
			public void run(){
        		...
        		Log.d(TAG, "begin completedTask but not install,completedTaskList.size="+completedTaskList.size());
...
//                	deleteTask(task);先不自动删除
                ...
                case WORKMSG_CONNECT_APKSERVICE:
					mng.apk(new PayMng.CallbackActionListener() {
						@Override
						public void execute() {
							Log.d(TAG,"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
							apkService = mng.getApkInfoService();
							Log.d(TAG,"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa apkService :"+ apkService);
						}
					});
            		break;
...
//						sendBroadcast(new Intent(Contants.INTENT_DOWNLOAD_COMPLETED));
			    	...
			    	tDBUtils.deleteOneApplication(tBean.getPkgName());/* while has downloadComplete but turn off */
			    	multiThreadMap.remove(tBean.getId());
			    	speedMap.remove(tBean.getId());
					Intent tIntent=new Intent();
					Intent installIntent=new Intent();
...
//		registerTaskObserver();
	    ...
	    mng = new PayMng(this.getApplicationContext());
	    mng.apk(new PayMng.CallbackActionListener() {
			@Override
			public void execute() {
				apkService = mng.getApkInfoService();
