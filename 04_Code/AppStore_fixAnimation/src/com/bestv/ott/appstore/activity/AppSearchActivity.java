...
import com.bestv.ott.appstore.thread.SearchAppThread;
...
public class AppSearchActivity extends BaseActivity implements OnFocusChangeListener,OnClickListener{
	...
	private boolean isPage=false;//表示正在翻页
	...
	public void initView(){//初始化
		...
		mButtonSearch = (ImageView) this.findViewById(R.id.search_button_search);
    ...
    public void refreshAppsList(List<AppsBean> appList){
    	Log.d(TAG, "refreshAppsList--"+appList.size());
    	mAppList=appList;
    	if(mAppList==null){
    		isPage=false;
    		return;
...
//				ab.setImageURI(Uri.parse(mAppList.get(i).getNatImageUrl()));
				Bitmap bm = CaCheManager.requestBitmap(mAppList.get(i).getNatImageUrl());
				if(bm!=null){
			...
			setPropertyIcon(view, mAppList.get(i));
		}
		if(mAppList.size()<3){
