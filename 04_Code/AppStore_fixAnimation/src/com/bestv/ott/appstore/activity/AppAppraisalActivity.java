...
public class AppAppraisalActivity extends Activity {
	...
	private static final int MSG_CONNECT_BESTV=11;
	//各个等级评分值
...
//		}
	...
	private void evaluated() {
		...
		mRatingBar.setRating(mScoreValue);
		mTextView1.setText(AppAppraisalActivity.this.getString(R.string.app_appraisal_text1,mAmount));
	...
	private class RatingBarListener implements
		...
		public void onRatingChanged(RatingBar ratingBar, float rating,
				boolean fromUser) {
			//AppLog.log_D(TAG, "--------------------RatingBar------正在评价-------");
			mIsMove = true;
	...
	protected void onPause() {
		...
		//if(mIsMove && mIsSuer){ 
		if(mIsSuer){ 
			AppLog.d(TAG, "----------------RatingBar is close!!!------The scoring results  :--"+mScoreValue*2+"---");
			new ScoreThread(mUserID, mAppId, mScoreValue*2,AppAppraisalActivity.this).start();
		}else{
			AppLog.d(TAG, "----------------------------May score or no score------------------------------------");
		}
