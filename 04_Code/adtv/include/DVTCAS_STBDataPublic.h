...
typedef struct _SDVTCACorresInfo{
	DWORD		m_tSetTime;				//设置时间。
	DWORD		m_dwMotherCardID;		//母卡编号。
	BYTE			m_byFlag;				//6 bits:保留；2bits使用标准标识。
	DWORD		m_dwDecryptCWTimes;		//解密CW的次数。
	DWORD		m_dwIntervalTime;		//收看间隔时间。
	DWORD		m_tCorrespondTime;		//子卡的配对时间。
}SDVTCACorresInfo;
